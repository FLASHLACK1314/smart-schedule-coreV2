...
import io.github.flashlack1314.smartschedulecorev2.mcp.dto.TeacherScheduleQueryDTO;
import io.github.flashlack1314.smartschedulecorev2.mcp.dto.TimeSlotCheckDTO;
...
public class EduScheduleTool {
    ...
    public String queryTeacherScheduleByTime(
        ...
        if (!StringUtils.hasText(teacherName)) {
            return toJson(new TeacherScheduleQueryDTO()
                    .setSuccess(false)
                    .setErrorMessage("请提供教师姓名。"));
        }
        ...
        // 2. 组装每个教师的课表数据
        List<TeacherScheduleQueryDTO.TeacherSchedule> teacherSchedules = new ArrayList<>();
        ...
        for (TeacherDO teacher : teachers) {
            ...
            List<TeacherScheduleQueryDTO.ScheduleItem> items = new ArrayList<>();
            for (ScheduleDO schedule : schedules) {
                items.add(new TeacherScheduleQueryDTO.ScheduleItem()
            ...
            String filterDesc = dayOfWeek != null ? getDayOfWeekStr(dayOfWeek) : "全部";
            teacherSchedules.add(new TeacherScheduleQueryDTO.TeacherSchedule()
                    .setTeacherUuid(teacher.getTeacherUuid())
                    .setTeacherName(teacher.getTeacherName())
                    .setTeacherNum(teacher.getTeacherNum())
                    .setFilterDescription(filterDesc)
                    .setScheduleCount(schedules.size())
                    .setSchedules(items));
        }

        return toJson(new TeacherScheduleQueryDTO()
                .setSuccess(true)
                .setTeachers(teacherSchedules));
    }
    ...
    public String checkTimeSlotAvailability(
        ...
        // 参数校验
        if (dayOfWeek == null || dayOfWeek < 1 || dayOfWeek > 7) {
            return toJson(new TimeSlotCheckDTO()
                    .setSuccess(false)
                    .setErrorMessage("星期几参数必须在1-7之间。"));
        }
        if (sectionStart == null || sectionEnd == null || sectionStart > sectionEnd) {
            return toJson(new TimeSlotCheckDTO()
                    .setSuccess(false)
                    .setErrorMessage("节次参数无效，起始节次不能大于结束节次。"));
        }
        if (!StringUtils.hasText(semesterUuid)) {
            return toJson(new TimeSlotCheckDTO()
                    .setSuccess(false)
                    .setErrorMessage("请提供学期UUID。"));
        }

        TimeSlotCheckDTO.TimeSlotInfo timeSlot = new TimeSlotCheckDTO.TimeSlotInfo()
                .setDayOfWeek(dayOfWeek)
                .setDayOfWeekStr(getDayOfWeekStr(dayOfWeek))
                .setSectionStart(sectionStart)
                .setSectionEnd(sectionEnd);

        List<TimeSlotCheckDTO.CheckResult> results = new ArrayList<>();
        ...
        if (StringUtils.hasText(classroomName)) {
            ...
            if (classroom == null) {
                results.add(new TimeSlotCheckDTO.CheckResult()
                        .setCheckType("classroom")
                ...
                TimeSlotCheckDTO.CheckResult checkResult = new TimeSlotCheckDTO.CheckResult()
                        .setCheckType("classroom")
                        .setName(classroom.getClassroomName())
                        .setFound(true)
                        .setHasConflict(hasConflict);

                if (hasConflict) {
                    hasAnyConflict = true;
                    conflictTypes.add("教室冲突");
                    ...
                    List<TimeSlotCheckDTO.ConflictItem> conflictItems = new ArrayList<>();
                    for (ScheduleDO s : conflictSchedules) {
                        conflictItems.add(new TimeSlotCheckDTO.ConflictItem()
                                .setCourseName(getCourseName(s.getCourseUuid()))
                                .setRelatedName(getTeacherName(s.getTeacherUuid())));
                    }
                    checkResult.setConflicts(conflictItems);
                }
            ...
            if (teacher == null) {
                ...
                if (hasConflict) {
                    ...
                    for (ScheduleDO s : conflictSchedules) {
                        conflictItems.add(new TimeSlotCheckDTO.ConflictItem()
                                .setCourseName(getCourseName(s.getCourseUuid()))
                                .setRelatedName(getClassroomName(s.getClassroomUuid())));
                    }
                    checkResult.setConflicts(conflictItems);
                }
                results.add(checkResult);
    ...
    /**
     * 将对象序列化为 JSON 字符串
     * 用于 MCP 工具返回 DTO 对象时，确保 Dify 将结果放入 text 字段
     */
    private String toJson(Object obj) {
        try {
            ...
            return "{\"success\":false,\"errorMessage\":\"服务端序列化错误\"}";
        }
    }
